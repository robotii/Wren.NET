...
namespace Wren.Core.Objects
{
    ...
    public class ObjMap : Obj
    {
        ...
        Dictionary<Value,Value> entries;

        // Looks up [key] in [map]. If found, returns the value. Otherwise, returns UNDEFINED.
        public Value Get(Value key)
        {
            Value v;
            return entries.TryGetValue(key, out v) ? v : new Value();
        }

        // Creates a new empty map.
        public ObjMap()
        {
            entries = new Dictionary<Value, Value>(new ValueComparer());
        ...
        public Value Get(int index)
        {
            if (index < 0 || index >= entries.Count)
                return new Value();
            Value[] v = new Value[entries.Count];
